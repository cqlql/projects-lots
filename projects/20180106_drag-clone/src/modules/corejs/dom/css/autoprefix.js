...
export default function (cssPropertyName) {
  // 如果有直接返回
  let propertyName = cache[cssPropertyName]
  if (propertyName) return propertyName

  let firstLetter = cssPropertyName[0]
  let firstLetterUpper = firstLetter.toUpperCase()
  let cssPrefixes = ['ms' + firstLetterUpper, 'Moz' + firstLetterUpper, 'webkit' + firstLetterUpper, firstLetter]
  let cssPrefixesReal = ['-ms-', '-Moz-', '-webkit-', '']
  let style = document.body.style
  // css名称转换
  let name = cssPropertyName.replace(/-\w/g, function (d) {
