...
></polyline>`
    // div文本
    if (exOft > 0) {
      texts += `<div class="lb" style="left:${(tx + 2) / 200 * 100}%;top:${(ty - 4) / 120 * 100}%;"><span style="color:${lcs[i % 5]}">${labels[i]}</span></div>`
    } else {
      texts += `<div class="lb" style="right:${(200 - tx + 2) / 200 * 100}%;top:${(ty - 4) / 120 * 100}%;"><span style="color:${lcs[i % 5]}">${labels[i]}</span></div>`
    }
    // svg 文本
  ...
  let textScore = `<text x="${ox}" y="${oy + 3}" font-size="6.5" text-anchor="middle" fill="#333" style="font-weight:bold;">${tscore}分</text>`
