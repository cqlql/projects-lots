    ...
    <div class="cont">
      <div class="img-box">
        ...
        <div class="cont-main">
          ...
          <div v-if="iptShow" ref="eIpt" class="ipt" :class="{'no-color':!isStop}" v-click="toFocus"><span>3.</span><span v-for="(v,i) in piVal" :class="{err:v!==d[i]}">{{v}}</span><span :class="{err:currPiValErr}">{{currPiVal}}</span><span class="cursor" :class="{flicker:cursorFlicker}"></span></div>
  ...
  export default {
    ...
    methods: {
      ...
      input (e) {
        ...
        let v = target.value
        if (this.isStop === 0 && v) {
          let {count, rightCount, wrongCount} = this
          v.split('').every(v => {
